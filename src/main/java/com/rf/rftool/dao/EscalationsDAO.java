...
public List<Escalations> getEscalationBySerach(String siteid, String sitename, String startdate, String enddate,
    ...
    if(siteid!="" && enddate!="" && startdate!="" && status!="" ) 
    {
    	String sql = "select * from ssaescalation where  siteid = '"+siteid+"' and  startdate between '"+startdate+"' and '"+enddate+"' and  status='"+status+"';";
        System.out.println(sql);	
        list = template.query("select * from ssaescalation where  siteid ='"+siteid+"' and  startdate between '"+startdate+"' and '"+ enddate +"' and  status='"+status+"' ;",new ResultSetExtractor<List<Escalations>>(){  
        	List<Escalations> list1=new ArrayList<Escalations>(); 
		     public List<Escalations> extractData(ResultSet rs) throws SQLException,  
		            DataAccessException {  
